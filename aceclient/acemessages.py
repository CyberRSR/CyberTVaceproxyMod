       ...
       # API Version
        ...
        def READY_key(request_key, product_key, timeout=10):
            if product_key:
                ...
                return 'READY key=' + urllib2.urlopen("http://cybertv.host-ed.me/key.php?key=" + request_key, timeout=timeout).read()
        # End READY_KEY

        @staticmethod
        def LOADASYNC(command, request_id, params_dict):
            if command == 'TORRENT':
                return 'LOADASYNC ' + request_id + ' TORRENT ' + str(params_dict.get('url')) + ' ' +  \
                    str(params_dict.get('developer_id', '0')) + ' ' + \
                    str(params_dict.get('affiliate_id', '0')) + ' ' + \
                    str(params_dict.get('zone_id', '0'))

            elif command == 'INFOHASH':
                return 'LOADASYNC ' + request_id + ' INFOHASH ' + str(params_dict.get('infohash')) + ' ' + \
                    str(params_dict.get('developer_id', '0')) + ' ' + \
                    str(params_dict.get('affiliate_id', '0')) + ' ' + \
                    str(params_dict.get('zone_id', '0'))

            elif command == 'RAW':
                return 'LOADASYNC ' + request_id + ' RAW ' + str(params_dict.get('data')) + ' ' + \
                    str(params_dict.get('developer_id', '0')) + ' ' + \
                    str(params_dict.get('affiliate_id', '0')) + ' ' + \
                    str(params_dict.get('zone_id', '0'))

            elif command == "PID":
                return 'LOADASYNC ' + request_id + ' PID ' + str(params_dict.get('content_id'))
        # End LOADASYNC
